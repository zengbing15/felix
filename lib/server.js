...
import {SerializeUnsignedTransaction} from "../schema/UnsignedTransaction.js";
...
const bot = botgram("1630901998:AAG0QkR_cb15E6dvLsj9v6MLGlmZJk2qakU",{ agent: utils.createAgent() });
...
const STATE_MACHINE = {
  ...
  [DATA_PAY_SIGNING]: async (session, msg, reply) => {
    ...
    // download the whole transaction json file
    const readable = toStream(Buffer.from(JSON.stringify(tx,null,2), 'utf8'));

    const writerStream = fs.createWriteStream('tx.json');

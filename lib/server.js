...
*/
/* TODO: for now we directly fill in the token value, will deal with this later. */
// Note:The token is the auth token of @ckbtestfelix_bot.
const bot = botgram("1520701607:AAEJEx2bXKNU822b3nsoBIWkioOAtuHoZp0",{ agent: utils.createAgent() });
...
bot.command("start", (_msg, reply) => {
  reply.text("Greetings! /help to see available commands!");
});
bot.command("help", (_msg, reply) => {
  reply.text(
    "/set_receiving_address - Set receiving address in a private chat\n" +
...
const STATE_MACHINE = {
  ...
  [DATA_PAY_SIGNING]: async (session, msg, reply) => {
    ...
    //reply.text(JSON.stringify(tx,null,2));
    reply.text(`Envelope successfully paid! TX hash: ${txHash}`);

    // download the whole tx json file 
    // TODO: Every time pay red packets,need to set the proxy to download the tx JSON file,will deal with it later.
    
    let data = JSON.stringify(tx,null,2);
    fs.writeFile('tx.json', data, (err) => {
      if (err) {
          throw err;
      }
